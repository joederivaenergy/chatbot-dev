...
# ============================================
# SIMPLE DEBUG MODE (DEVELOPER ONLY)
# ============================================
...
def extract_query_info(user_query: str) -> Dict:
    ...
    debug_print("USER QUERY", user_query)
    
    response = call_claude(EXTRACTION_PROMPT, user_query, include_history=True)
    
    debug_print("LLM EXTRACTION RAW RESPONSE", response)
    ...
    try:
        ...
        debug_print("PARSED EXTRACTION", data)
        ...
        debug_print("FINAL MERGED CONTEXT", merged)
...
def search_descriptions_by_keywords(team: str, keywords: str, operation_type: str = None) -> List[str]:
    ...
    if team_key not in ALL_TEAM_DATA or ALL_TEAM_DATA[team_key].empty:
        debug_print("ERROR", f"CSV not found: {team_key}")
        return []
    ...
    debug_print("SEARCH WORDS", search_words)
    ...
    result = sorted(list(matching_descriptions))
    
    debug_print("MATCHING DESCRIPTIONS FOUND", {
        "count": len(result),
        "descriptions": result[:10]  # Show first 10
    })
...
def get_charging_data(team: str, exact_description: str, location: str = None, operation_type: str = None) -> Tuple[pd.DataFrame, bool]:
    ...
    debug_print("FINAL RESULT", {
        "rows": len(matches),
        "data": matches.to_dict('records') if len(matches) <= 3 else "Too many to show"
    })
...
def process_charging_question(user_input: str) -> str:
    ...
    debug_print("PROCESS CHARGING QUESTION", user_input)
    ...
    if is_likely_new_query(user_input):
        debug_print("NEW QUERY DETECTED", "Resetting context")
        st.session_state.extracted_context = {
            "team": None,
            "keywords": None,
            "location": None,
