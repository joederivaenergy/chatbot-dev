...
class DynamoDBChatHistory:
    ...
    # ----------------------------------------------------------------
    # FIX #1 — replaces get_formatted_history()
    # Old approach flattened all history into a single user text block,
    # which caused Claude to lose conversation context after a few turns.
    # This returns a proper alternating messages[] array for Bedrock.
    # ----------------------------------------------------------------
    def get_messages_for_bedrock(self) -> List[Dict]:
        ...
        for msg in messages[-20:]:  # keep last 20 messages for context window
            ...
            # Normalize role names stored in DynamoDB
        ...
        # Bedrock requires strictly alternating roles — merge any consecutive same-role messages
        cleaned = []
...
/* FIX #2 — pin file uploader just above the chat input bar, never moves */
.pinned-uploader {
    position: fixed;
    bottom: 70px;          /* sits just above st.chat_input bar (~60px tall) */
    left: 50%;
    transform: translateX(-50%);
    width: min(698px, 90vw); /* matches Streamlit centered layout max-width */
    z-index: 100;
    background: white;
    padding: 4px 0 2px 0;
}
...
def call_claude(system_prompt: str, user_message: str, include_history: bool = True) -> str:
    ...
    FIX #1: Uses get_messages_for_bedrock() to pass a proper alternating
    messages[] array instead of flattening history into a single user message.
    FIX #3: max_tokens raised to 8192 for longer responses and better memory.
    """
    try:
        if include_history:
            # Build proper messages array from stored history
            messages = chat_history.get_messages_for_bedrock()
            # Append the new user message
            messages.append({"role": "user", "content": user_message})
            # Final safety pass: ensure no consecutive same-role messages
            cleaned = []
            for msg in messages:
                if cleaned and cleaned[-1]["role"] == msg["role"]:
                    cleaned[-1]["content"] += "\n" + msg["content"]
        ...
        response = bedrock_runtime.invoke_model(
            ...
            body=json.dumps({
                ...
                "max_tokens": 8192,   # FIX #3: raised from 4096
                "system": system_prompt,
                "messages": messages  # FIX #1: proper messages array
            })
        )
...
def call_claude_with_media(system_prompt: str, user_message: str, uploaded_files: list) -> str:
    ...
    try:
        ...
        # FIX #1: use proper history array here too
        ...
        response = bedrock_runtime.invoke_model(
            ...
            body=json.dumps({
                "anthropic_version": "bedrock-2023-05-31",
                "max_tokens": 8192,   # FIX #3: raised from 4000
                "system": system_prompt,
                "messages": messages
            })
        )
...
# FIX #2: File uploader pinned just above the chat input bar via fixed CSS.
# Wrapped in .pinned-uploader div which uses position:fixed so it never
# moves regardless of how long the conversation grows.
uploaded_files = None
if st.session_state.chat_mode == "general":
    st.markdown('<div class="pinned-uploader">', unsafe_allow_html=True)
